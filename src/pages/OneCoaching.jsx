...
const OneCoaching = () => {
  ...
  const fetchClients = async () => {
    try {
      const res = await axios.get(`http://localhost:8080/api/coaching/${trainer._id}/clients`);
      setClients(res.data);
    } catch (err) {
  ...
  const handleRemove = async (clientId) => {
    try {
      await axios.delete(`http://localhost:8080/api/coaching/${trainer._id}/${clientId}`);
