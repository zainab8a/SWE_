...
const CommunityManagement = () => {
  ...
  const handleCreate = async () => {
    ...
    const res = await axios.post('http://localhost:8080/api/communities', newCommunity);
  ...
  const handleAddMember = async () => {
    ...
    try {
      const res = await axios.put(`http://localhost:8080/api/communities/${selectedCommunity._id}/add-member`, {
  ...
  const handleRemoveMember = async (userId) => {
    const res = await axios.put(`http://localhost:8080/api/communities/${selectedCommunity._id}/remove-member`, { userId });
  ...
  const handleAddChallenge = async () => {
    ...
    const res = await axios.put(`http://localhost:8080/api/communities/${selectedCommunity._id}/add-challenge`, challenge);
    setSelectedCommunity(res.data);
    setCommunities(communities.map(c => c._id === res.data._id ? res.data : c));
  ...
  const handleRemoveChallenge = async (title) => {
    const res = await axios.put(`http://localhost:8080/api/communities/${selectedCommunity._id}/remove-challenge`, { title });
    setSelectedCommunity(res.data);
    setCommunities(communities.map(c => c._id === res.data._id ? res.data : c));
  };
